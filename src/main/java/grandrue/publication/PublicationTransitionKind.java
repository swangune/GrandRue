...
/** Publication lifecycle-history event kinds required by MS-PROT-046. */
public enum PublicationTransitionKind {
    ESTABLISH,
    PUBLISH,
    WITHDRAW,
    REPUBLISH;
    ...
    /**
     * Classifies an already-valid same-Opportunity state change without turning persistence into
     * lifecycle policy. Material revision-only advances intentionally produce no lifecycle event.
     */
    public static Optional<PublicationTransitionKind> between(
    ...
    private static void requireSamePublication(
        ...
        if (!expectedState.merchantScope().equals(nextState.merchantScope())
                || !expectedState.opportunityIdentity().equals(
                        nextState.opportunityIdentity()
                )) {
            throw new IllegalArgumentException(
