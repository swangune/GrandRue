...
public final class BackgroundWorkContractRegistrySnapshot {
    ...
    public BackgroundWorkContractRegistrySnapshot(String semanticRegistryReleaseIdentifier,
        ...
        var index = new HashMap<BackgroundWorkContractIdentity, BackgroundWorkContractDefinition>();
        for (var definition : Objects.requireNonNull(definitions, "definitions")) {
            ...
            if (index.putIfAbsent(definition.identity(), definition) != null)
                throw new IllegalArgumentException("Duplicate BackgroundWorkContract identity");
        }
        contracts = Map.copyOf(index);
    }

    public String semanticRegistryReleaseIdentifier() { return release; }
    public Set<BackgroundWorkContractDefinition> contracts() { return Set.copyOf(contracts.values()); }

    public Optional<BackgroundWorkContractDefinition> contract(String requestedRelease, BackgroundWorkContractIdentity identity) {
        BackgroundWorkContractIdentity.requireReference(requestedRelease, "requested semantic release");
        Objects.requireNonNull(identity, "identity");
        return release.equals(requestedRelease) ? Optional.ofNullable(contracts.get(identity)) : Optional.empty();
    }
}
