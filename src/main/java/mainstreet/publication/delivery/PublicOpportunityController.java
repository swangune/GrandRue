...
public final class PublicOpportunityController {
    ...
    public PublicOpportunityController(PublicOpportunityQuery query) { this.query = Objects.requireNonNull(query); }

    @GetMapping("/api/public/storefronts/{locator}/opportunities/{opportunity}")
    public ResponseEntity<?> get(@PathVariable String locator, @PathVariable String opportunity,
                                 @RequestParam Map<String, String> parameters) {
        if (!parameters.isEmpty()) return problem(ApiProblemCategory.INVALID_REQUEST);
        try {
            ...
            return response.<ResponseEntity<?>>map(value ->
                    ...
                    .orElseGet(() -> problem(ApiProblemCategory.NOT_FOUND_OR_NOT_ACCESSIBLE));
    ...
    private static ResponseEntity<ApiProblem> problem(ApiProblemCategory category) {
