...
| MS-PROT-051-V11-DQ-003 | **DEFERRED — NARROWED TO IMPLEMENTATION BEFORE PUBLIC NAVIGATION** | Exact geocoding/map provider, coordinate-normalisation implementation and provider-binding lifecycle | MS-PROT-045 v1.2 resolves the semantic boundary: provider response/identifier do not own business truth; precise navigation point is provider-neutral accepted domain value; maps, directions and street-level imagery are derived presentation. Concrete provider selection, geocoding implementation, coordinate-normalisation implementation and provider-binding lifecycle remain under provider/location implementation | Must be completed before production public-navigation integration is claimed conforming; provider selection remains replaceable and must not become Merchant Location or subject semantic authority |
...
MS-PROT-044 v1.6, accepted 19 September 2026, resolves the current Listing commercial-classification gap: `listing/merchant-definition-authoring-access@1` / `MAINTAIN_MERCHANT_LISTING_DEFINITION` is protected for FREE + BUSINESS + GROWTH, while `listing/merchant-definition-observation-access@1` and `listing/merchant-withdrawal-access@1` require no independent Commercial Entitlement. The classification preserves Listing/subject ownership, terminal withdrawal, Category, Physical Location, Exposure, Storefront and downstream transaction boundaries; it creates no final entitlement identity, does not publish a catalogue and activates no implementation. DQ-001 remains OPEN for final completeness audit, concrete entitlement identities, exact bindings and the complete standard manifest.

MS-PROT-045 v1.2, accepted 19 September 2026, resolves the retained MS-PROT-044 `location/coordinates model` and `map provider integration` semantic questions within the source/presentation boundary. It establishes reusable `PhysicalLocationBindingV1` with Merchant Location reference or independent physical-place forms, reuses `PostalAddressV1`, establishes a provider-neutral `AcceptedNavigationPoint`, keeps provider identifiers/evidence non-authoritative, and makes map/directions/street-level imagery derived Projection/Exposure/Storefront presentation. `MS-PROT-051-V11-DQ-003` remains open only for concrete provider selection, geocoding/coordinate-normalisation implementation and provider-binding lifecycle. No independent Commercial Entitlement or implementation activation is introduced.

MS-PROT-057 v1.3, accepted 19 September 2026, resolves the final owner/supporting commercial-classification gap for the currently admitted standard portfolio. Exact `merchant-assistance/supporting-interaction-access@1` is classified as `PLATFORM_SERVICE_ACCESS` with `NO INDEPENDENT COMMERCIAL ENTITLEMENT`, bounded to the accepted v1.2 merchant-assistance responsibility portfolio. It creates no generic AI entitlement, preserves underlying capability/actor/commercial/exposure/provider authority, and does not activate implementation. The DQ-001 classification audit is therefore complete; DQ-001 remains OPEN only for concrete entitlement identities, exact Commercial Access Bindings, complete explicit FREE/BUSINESS/GROWTH grant snapshots, manifest approval and publication-ready manifest construction.

MS-PROT-056 v1.10, accepted 19 September 2026, resolves `MS-PROT-056-V17-DQ-001` by approving the exact initial immutable standard Commercial Catalogue Manifest: `standard-commercial-catalogue@1`, exact plan revisions `standard-plan/free@1`, `standard-plan/business@1` and `standard-plan/growth@1`, 36 stable `CommercialEntitlementIdentity` definitions with exact owner-qualified bindings, explicit FREE/BUSINESS/GROWTH grant snapshots of 13/34/36 identities satisfying strict monotonicity, and the required conditional supporting-commercial relationships. It excludes unresolved reservations and no-independent-entitlement supporting paths, leaves DQ-003 and the remaining DQ-004 reservations unchanged, and approves manifest content without executing `PublishStandardCommercialCatalogue` or activating implementation.
...
|---|---|---|---|
| MS-PROT-056-V17-DQ-001 | **RESOLVED** | Exact entitlement definitions, target bindings and complete standard catalogue | Resolved by MS-PROT-056 v1.10: exact `standard-commercial-catalogue@1`, three exact plan revision identities, 36 stable entitlement identities and exact owner-qualified bindings, explicit 13/34/36 FREE/BUSINESS/GROWTH snapshots, strict monotonicity and owner-qualified conditional supporting-commercial requirements. Acceptance approves the manifest but does not publish it or activate implementation; DQ-003 pricing/allowances and remaining DQ-004 reservations stay separately open |
