...
**Product identity resolution:** **GrandRue** is the current product and repository identity. Within current accepted authority, legacy product-name references to `Main Street` refer to GrandRue unless the context is explicitly historical. Stable `MS-*` authority identifiers, `mainstreet.*` implementation/package identifiers and historical provenance remain unchanged until a separately governed migration changes them.
